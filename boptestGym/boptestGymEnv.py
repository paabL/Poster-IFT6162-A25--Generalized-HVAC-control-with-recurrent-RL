...
class BoptestGymEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.objective_indiv_integrand = np.zeros(2) #TODO : rendre general
    ...
    def step(self, action):
        ...
        indiv_reward = self.get_indiv_reward()
        self.episode_indiv_rewards.append(indiv_reward)
        
        reward = np.sum(indiv_reward)
    ...
    def render(self, mode='episodes'):
        ...
        if mode != 'episodes':
            ...
            self.fig = plt.gcf()
            self.fig.clear()
            plot_results(self, self.episode_rewards, log_dir=self.log_dir, indiv_rewards=self.episode_indiv_rewards)
    ...
    def get_indiv_reward(self):
        ...
        objective_indiv_integrand = np.array([w1*kpis['cost_tot'], w2*kpis['tdis_tot']])
        
        # Compute reward
        indiv_reward = -(objective_indiv_integrand - self.objective_indiv_integrand)
        
        self.objective_indiv_integrand = objective_indiv_integrand
