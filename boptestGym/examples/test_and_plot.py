...
def plot_results(env, rewards, points=['reaTZon_y','reaTSetHea_y','reaTSetCoo_y','oveHeaPumY_u',
                                       'weaSta_reaWeaTDryBul_y', 'weaSta_reaWeaHDirNor_y', "oveTSet_u"],
                 log_dir=os.getcwd(), model_name='last_model', save_to_file=False, indiv_rewards=None):  #Changer le nom
    ...
    t      = np.arange(df['time'][0], env.start_time+env.max_episode_length, env.step_period) / 86400.
    res_t  = np.asarray(df['time']) / 86400.
    interp = lambda y: interpolate.interp1d(t, y, kind='zero', fill_value='extrapolate', axis=0)(res_t)

    rewards_reindexed       = interp(rewards)
    indiv_rewards_reindexed = None if indiv_rewards is None else interp(indiv_rewards)
    ...
    axs[0].plot(x_time, df["oveTSet_u"]  -273.15, color='darkred',   linestyle='--', linewidth=1, label='_nolegend_')
    ...
    if indiv_rewards is not None:
        for i, (c, lab) in enumerate(zip('rg', ['rewards_cost', 'rewards_confort'])):
            axs[2].plot(x_time, indiv_rewards_reindexed[:, i], c, linewidth=1, label=lab)
    axs[2].legend(loc='lower left', fontsize=4)
